...
import { Material, MaterialType, Supplier } from '../types'; // Ensure Supplier is imported
...
const MaterialLabelCanvas: React.FC<MaterialLabelCanvasProps> = ({ material, allSuppliers, onCanvasReady }) => {
  ...
  useEffect(() => {
    ...
    const drawLabel = () => {
      ...
      const headerBg = '#334155'; // primary (slate-700) - For print, dark header is fine
      const headerTextC = '#FFFFFF';
      const textColor = '#1e293b'; // neutral-dark (slate-800)
      const detailColor = '#475569'; // slate-600
      const accentColor = '#0d9488'; // accent (teal-600)
      ...
      ctx.strokeStyle = '#e2e8f0'; // neutral-medium (slate-200)
      ctx.lineWidth = 1;
      ...
      // ID Bar background
      ctx.fillStyle = '#e2e8f0'; // neutral-medium (slate-200)
  ...
  // The canvas element itself gets dark mode border to fit in the page.
  // The content of the canvas is drawn for light mode (printability).
  return (
    <canvas 
        ref={canvasRef} 
        width={300} 
        height={450} 
        className="border border-border-default dark:border-slate-700 rounded-md shadow-md bg-white" // bg-white ensures the canvas visual background is white even in dark mode
    />
  );
};
